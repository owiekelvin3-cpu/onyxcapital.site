...
import {
  CUSTOM_REJECTION_REASON_ID,
  WITHDRAWAL_REJECTION_REASONS,
  getWithdrawalRejectionReasonText,
} from "@/lib/withdrawal-rejection-reasons";
import { X } from "@/components/icons";

export function AdminWithdrawalRejectDialog({
  open,
  amount,
  userLabel,
  busy,
  ...
  const [reasonId, setReasonId] = useState<(typeof WITHDRAWAL_REJECTION_REASONS)[number]["id"] | typeof CUSTOM_REJECTION_REASON_ID>(
    WITHDRAWAL_REJECTION_REASONS[0].id
  );
  ...
  useEffect(() => {
    if (!open) return;
    setReasonId(WITHDRAWAL_REJECTION_REASONS[0].id);
    setCustomReason("");
  }, [open]);

  if (!open || typeof document === "undefined") return null;

  const resolvedReason = getWithdrawalRejectionReasonText(reasonId, customReason);
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-border px-5 py-4">
          <div className="min-w-0">
            <h2 id="reject-withdrawal-title" className="text-base font-semibold text-text-primary">
              Reject withdrawal
            </h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="space-y-2">
            {WITHDRAWAL_REJECTION_REASONS.map((reason) => (
              <label
                key={reason.id}
                className={cn(
              ...
              >
                <input
                  type="radio"
                  name="withdrawal-rejection-reason"
                  className="mt-1"
            ...
            <label
              className={cn(
                "flex cursor-pointer gap-3 rounded-xl border px-3 py-3 transition-colors",
                reasonId === CUSTOM_REJECTION_REASON_ID
                  ? "border-brand/40 bg-brand/5"
                  : "border-border bg-bg-primary/60 hover:border-border-light"
              )}
            >
              <input
                ...
                name="withdrawal-rejection-reason"
                className="mt-1"
                checked={reasonId === CUSTOM_REJECTION_REASON_ID}
                onChange={() => setReasonId(CUSTOM_REJECTION_REASON_ID)}
              />
