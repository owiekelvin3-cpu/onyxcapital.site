...
function priceClass(direction?: "up" | "down" | "flat"): string {
  if (direction === "up") return "text-green";
  if (direction === "down") return "text-red";
  return "text-text-primary";
}

export function MemeCoinLiveCard({ coin }: { coin: LiveMemeCoin | MemeCoinRow }) {
  const live = coin as LiveMemeCoin;
  const price = live.livePriceUsd ?? Number(coin.price_usd ?? 0);
  ...
  return (
    ...
    >
      <div className="absolute right-3 top-3 flex items-center gap-1.5">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-wide text-green">Live</span>
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="text-right">
          <p className={cn("text-sm font-semibold font-mono transition-colors duration-300", priceClass(live.priceDirection))}>
            {formatMemePrice(price)}
          </p>
          <p className={cn("text-xs font-semibold font-mono", positive ? "text-green" : "text-red")}>
...
export function MemeCoinLiveGrid({ coins }: { coins: (LiveMemeCoin | MemeCoinRow)[] }) {
  if (coins.length === 0) {
    return (
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {coins.map((coin) => (
        <MemeCoinLiveCard key={coin.id} coin={coin} />
      ))}
    </div>
  );
}
