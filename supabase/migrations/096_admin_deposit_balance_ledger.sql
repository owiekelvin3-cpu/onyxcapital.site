...
-- Track admin Deposit balance credits separately so they are not absorbed by Profit Total.
...
BEGIN
  ...
  v_credits := COALESCE((SELECT SUM(a.amount) FROM public.user_deposit_adjustments a WHERE a.user_id = p_user_id), 0);
