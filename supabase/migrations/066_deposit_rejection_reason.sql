-- Store a client-facing reason when a deposit is rejected.

ALTER TABLE public.deposits
  ADD COLUMN IF NOT EXISTS rejection_reason text;

CREATE OR REPLACE FUNCTION public.notify_deposit_changes()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    ELSIF NEW.status = 'rejected' THEN
      ...
      PERFORM create_notification(
        NEW.user_id,
        'Deposit rejected',
        'Your deposit of ' || amt || ' via ' || NEW.method || ' was not approved.'
        || CASE
